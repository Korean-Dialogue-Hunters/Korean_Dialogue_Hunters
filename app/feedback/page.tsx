...
const MOCK_FEEDBACK: FeedbackData = {
  ...
  messages: [
    { sender: "user", text: "안녕하세요! 한강에서 가요?", hasError: true, errorHighlights: ["한강에서 가요"] },
    { sender: "ai", text: "안녕하세요! 네, 한강에 가고 있어요. 날씨가 좋네요!", hasError: false },
    { sender: "user", text: "자전거 어디 있어?", hasError: true, errorHighlights: ["어디 있어?"] },
    { sender: "ai", text: "저기 편의점 옆에 자전거 대여소가 있어요!", hasError: false },
    { sender: "user", text: "아 감사합니다! 같이 타볼까요?", hasError: false },
    { sender: "ai", text: "좋아요! 같이 타요. 어디로 갈까요?", hasError: false },
    { sender: "user", text: "여의도 쪽으로 가서 맛있는 거 먹자", hasError: true, errorHighlights: ["맛있는 거 먹자"] },
    { sender: "ai", text: "좋은 생각이에요! 여의도에 유명한 치킨집이 있어요.", hasError: false },
  ],
};
...
function FeedbackBubble({ msg }: { msg: FeedbackMessage }) {
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"} mb-3`}>
      ...
      >
        {isUser && msg.hasError ? (
          <HighlightedText text={msg.text} highlights={msg.errorHighlights} />
        ) : (
          msg.text
        )}
      </div>
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-tab-inactive mb-6">
        총점 <span className="font-bold text-accent">{feedback.totalScore}</span> / 10
      </p>
